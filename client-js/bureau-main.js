var bureau = {
	...
	refreshNotifications: function() {
		...
		bureau.api(bureau.user.uid, 'read', 'notifications', {
			limit: window.innerHeight / 60 + 5
	...
	displayNotifications: function() {
		...
		$(list).children().on('click', function(e) {
			...
			if (this.className.indexOf('unread') > -1) {
				...
				bureau.api(bureau.user.uid, 'write', 'readNotification', {
					id: id
				}, function(err, response) {
					var i = 0,
	...
	setup: {
		...
		guild: function() {
			...
			$('input[type=submit][name=resetpassword]').on('click', function(e) {
				...
				if (confirm('Are you sure you want to reset their password?')) {
					bureau.api(bureau.uid, 'write', 'resetPassword', {
						resetuid: resetuid
					}, function(err, j) {
						prompt(
		...
		guildNewGame: function() {
			...
			function displayGameSetupFragment(e) {
				...
				if (!!gtype) {
					...
					bureau.api(bureau.user.uid, 'read', 'gamesetupfragment', {
						gametype: gtype
					}, function(err, j) {
						if (!err) {
							el.className = ''
		...
		guildGameState: function() {
			...
			var loadGameState = function(el) {
					...
					if (clicked.indexOf(k) < 0) {
						...
						clicked.push(k)
						console.log(k)
						bureau.api(bureau.user.uid, 'read', 'gamestatefragment', {
							gameid: gameid,
							uid: uid
						}, function(err, j) {
		...
		personal: function() {
			...
			$I('opt-in-input').addEventListener('click', function(e) {
				...
				bureau.api(bureau.user.uid, 'write', 'setOptout', {
					optout: optout
				}, function(err, response) {
